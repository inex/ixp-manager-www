<?php $active = 'support'; include 'header.php' ?>

      <div class="row marketing">
        <div class="col-lg-12">

            <h2>Presentations and Talks</h2>


<p>
    Barry O&#8217;Donovan and Nick Hilliard routinely give talks and workshops on IXP Manager. We&#8217;ve published the more recent ones below and, where video exists, we&#8217;ll also link to that.
</p>

<h3>2018</h3>
<br>

<h4>
    APRICOT 2018 / APIX - IXP Manager Workshop.<br />
    <small>Kathmandu, Nepal. February 25th 2018.</small>
</h4>

<ul>
    <li>
        IXP Manager Workshop: Introduction
            [<a href="media/2018/inex-apricot-ixp-manager-workshop1-introduction-2018-02-25.pdf">pdf</a>]
            [<a href="https://youtu.be/l86wz0tvsQg?t=9m">video</a>]<br>
        Workshop introduction, an update on IXP Manager and an overview of its features.
    </li>
    <li>
        IXP Manager Workshop: Installation, Configuration
        [<a href="media/2018/inex-apricot-ixp-manager-workshop2-installation-2018-02-25.pdf">pdf</a>]
    </li>
</ul>
<br>



<h3>2017</h3>
<br>


<h4>
    RIPE 75<br />
    <small>Dubai, UAE. October 2017.</small>
</h4>

<ul>
    <li>
        <a href="media/2017/201710-ripe75.pdf">
            IXP Automation
        </a>
        [<a href="https://ripe75.ripe.net/archives/video/125">video</a>]
    </li>
</ul>
<br>


<h4>
    31st Euro-IX Forum<br />
    <small>Bratislava, Slovakia. October 2017.</small>
</h4>

<ul>
    <li>
        <a href="media/2017/inex-euro-ix-ixp-automation-2017-10-17.pdf">
            IXP Automation
        </a>
    </li>
</ul>
<br>


<h4>
    NLNOG Day 2017<br />
    <small>Amsterdam, Netherlands.September 8th 2017.</small>
</h4>

<ul>
    <li>
        <a href="media/2017/201709-nlnog2017-automation-with-ixp-manager.pdf">
            Automation with IXP Manager
        </a>
        [<a href="https://youtu.be/UAoLASkndaY">video</a>]
    </li>
</ul>
<br>


<h4>
    GPF 12.0 - Global Peering Forum<br />
    <small>New York City, USA. April 24-27th 2017.</small>
</h4>

<ul>
    <li>
        <a href="media/2017/201704-gpf12-ixpmanager.pdf">
            IXP Manager Update
        </a>:
        As the project turns ten this year, it is a fitting time to deliver our first
        States-side presentation and update the community on some major
        milestone goals.
    </li>
</ul>
<br>

<h4>
    30th Euro-IX Forum - Lightning Talks.<br />
    <small>Barcelona, Spain. April 9-11th 2017.</small>
</h4>

<ul>
    <li>
        <a href="media/2017/201704-euroix30-ixpmanager-xconnects.pdf">
            Cross Connect / Patch Panel Management
        </a>:
        A ~5 minute lightning talk to introduce the new cross connect / patch panel management feature.
        The <a href="https://docs.ixpmanager.org/features/patch-panels/">official
        documentation for this feature</a> would be recommended reading.
    </li>
</ul>
<br>

<h3>2016</h3>
<br>

<h4>
    28th Euro-IX Forum - 3rd IXP Manager Workshop.<br />
    <small>Luxembourg, Luxembourg. April 24th 2016.</small>
</h4>

<ul>
    <li>
        <a href="media/2016/201604-euroix28-ixpmanager-workshop-intro.pdf">
            Workshop Introduction and Update
        </a>:
        Workshop introduction, an update on IXP Manager development and an introduction to the new graphing service - Grapher.
    </li>
    <li>
        <a href="media/2016/201604-euroix28-grapher-anatomy-of-a-request.pdf">
            Grapher: Anatomy of a Request
        </a>:
        This presentation was a walk-through of how Grapher processes a request. There are essentially two sides to this: the HTTP request processing (including validation and access control) and the Grapher backend for processing a graph.
    </li>
    <li>
        <a href="media/2016/201604-euroix28-ixpmanager-v4-install-upgrade.pdf">
            IXP Manager V4 Installation/Upgrade
        </a>
        The purpose of this presentation was to give attendees an idea of what’s involved and how things are progressing towards an official release of IXP Manager v4.
    </li>
</ul>
<br>

<h3>2015</h3>
<br>

<h4>
    27th Euro-IX Forum - 2nd IXP Manager Workshop. <br />
    <small>Berlin, Germany. October 25th 2015.</small>
</h4>

<ul>
    <li>    Watch the workshop:
        <a href="https://www.youtube.com/watch?v=v8PRUQlRMRM">
            https://www.youtube.com/watch?v=v8PRUQlRMRM
        </a>
    </li>
    <li>
        <a href="media/2015/euroix27-inex-ixp-manager-workshop-introduction.pdf">
            Introduction to IXP Manager
        </a>
    </li>
    <li>
        <a href="media/2015/euroix27-inex-ixp-manager-v4-deep-dive.pdf">
            A Deep Dive into IXP Manager v4
        </a>
    </li>
</ul>
<br>
<h4>
    26th Euro-IX Forum - 1st IXP Manager Workshop.<br />
    <small>Marseille, France. April 12th 2015.</small>
</h4>

<ul>
    <li>
        Introduction to IXP Manager.
        This was repeated by atendee request at the 2nd IXP Manager workshop.
        See <a  href="media/2015/euroix27-inex-ixp-manager-workshop-introduction.pdf">
            the slides from that workshop
        </a> which were updated.
    </li>
</ul>

        </div>

<?php include 'footer.php'; ?>
